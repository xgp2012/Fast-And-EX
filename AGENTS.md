...
Minecraft 1.8.9 Chinese PvP client (named "Chocolate") built on Tritium-X. Java + Gradle using the custom `chocolate_gradle` plugin (not the standard `application` plugin).
...
./gradlew buildArch          # release zip -> build/cache/Chocolate.zip
```
...
- Release flow is CI-only: push a `v*` tag → `check` then `buildArch` → GitHub release of `build/cache/Chocolate.zip` (see `.github/workflows/release.yml`).
